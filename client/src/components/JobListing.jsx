...
const JobListing = () => {
    ...
    useEffect(() => {
        ...
        // Filter jobs based on search bar as it will take title
                ...
                {/* Pagination */}
                {
                    filteredJobs.length > 0 && (
                        <div className='flex items-center justify-center space-x-2 mt-10'>
                            ...
                            <a href="#job-list">
                                <img className={`${currentPage === Math.ceil(jobs.length / 6) ? 'opacity-50 cursor-not-allowed pointer-events-none' : 'cursor-pointer'}`} onClick={() => setCurrentPage(Math.min(currentPage+1, Math.ceil(filteredJobs.length/6)))} src={right_arrow_icon} alt="" />
                            </a>
                        </div>
                    )
                }
